...
async function checkNumberOfCompletedTodosInLocalStorage (page, expected) {
  return await page.waitForFunction(elementCount => {
    return JSON.parse(window.localStorage['react-todos']).filter((/** @type {ReactTodos} */ todo) => todo.completed).length === elementCount
  }, expected)
}
...
async function checkTodosInLocalStorage (page, title) {
  return await page.waitForFunction(text => {
    return JSON.parse(window.localStorage['react-todos']).map((/** @type {ReactTodos} */ todo) => todo.title).includes(text)
  }, title)
}
